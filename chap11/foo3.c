/*
* 
* 粗颗粒度锁：1）hashlock不仅负责散列表的结构组织、列表中节点的添加删除的加锁,
*				还负责foo结构体内部的count引用计数的加锁工作
*			 2）而foo结构中的lock负责与自身除pnext和引用计数count之外的其他数据的加锁
*			 相较于之前的情况foo2.c，该程序foo3.c中hashlock这个锁的管理范围增大，而
*				foo.lock这个内部锁的管理的范围缩小
* 
*/
